...
class Almacen extends Model
{
    ...
    public static function get_almacenes_by_id_empresa($id_empresa): ?array
    {
        $result = DB::select("EXEC sp_get_almacenes_by_id_empresa @id_empresa = ?", [$id_empresa]);
    ...
    public static function existencia_almacen_by_id($id_almacen, $id_empresa): ?bool
    {
        $result = DB::select("EXEC sp_existencia_almacen_by_id @id_almacen = ?, @id_empresa = ?", [$id_almacen, $id_empresa]);
        if (isset($result[0]->verificar)) {
            return $result[0]->verificar === 'true';
        }
        return null;
